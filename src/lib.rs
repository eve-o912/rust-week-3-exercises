...
impl Script {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        let (len_cs, len_consumed) = CompactSize::from_bytes(bytes)?;
        let total_len = len_cs.value as usize;
        if bytes.len() < len_consumed + total_len {
            return Err(BitcoinError::InsufficientBytes);
        }
        let script_bytes = bytes[len_consumed..len_consumed + total_len].to_vec();
        Ok((Script::new(script_bytes), len_consumed + total_len))
    }
}
...
impl TransactionInput {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        let offset = outpoint_consumed + script_consumed;
...
impl BitcoinTransaction {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        let (cs, cs_consumed) = CompactSize::from_bytes(&bytes[4..])?;
        let mut offset = 4 + cs_consumed;
